...
**MVP 구현** — 이번 턴 **손패 전체**를 액션 페이즈에서 사용. 슬롯 번호 키 대신:

```text
마우스 휠: 손패에서 사용할 카드 선택 (한 장만 활성)
마우스 좌클릭: 선택 카드 1회 사용 (동시에 1장만 처리)
remainingUses -= 1 → 0이면 (소진), 휠로 다른 카드 선택
```
